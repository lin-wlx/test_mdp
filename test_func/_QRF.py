...
class BaseForestQuantileRegressor(ForestRegressor):
    def fit(self, X, y):
        ...
        for i, est in enumerate(self.estimators_):
            ...
            # Ensure the estimator has y_train_leaves_ attribute
            # If ForestRegressor built the tree directly, we need to set it up
            if not hasattr(est, 'y_train_leaves_'):
                # Tree was built by sklearn, compute y_train_leaves_ from the training data
                est.y_train_leaves_ = est.tree_.apply(X[bootstrap_indices])
    ...
    def predict(self, X, uv=0): # , cos_sin
        ...
        if uv == 0: # debug. E(X_t|X_t-1). for CV too.
...
class RandomForestQuantileRegressor(BaseForestQuantileRegressor):
    ...
    def __init__(self,
                 n_estimators=10,
                 criterion='squared_error',  # 'mse' deprecated in sklearn 1.3+, use 'squared_error'
                 max_depth=None,
                 min_samples_split=2,
                 min_samples_leaf=1,
                 ...
                 verbose=0,
                 warm_start=False):
        # Store parameters for tree construction
        ...
        # Handle sklearn version compatibility for ForestRegressor initialization
        # In sklearn 1.4+, ForestRegressor accepts estimator parameter
        # The estimator should be fully configured with all parameters
        # Note: DecisionTreeRegressor doesn't accept max_features='auto', use None instead
        dt_max_features = max_features if max_features != 'auto' else None
        
        # Map deprecated criterion names to new ones for sklearn 1.3+
        criterion_mapped = criterion
        if criterion == 'mse':
            criterion_mapped = 'squared_error'
        elif criterion == 'mae':
            criterion_mapped = 'absolute_error'
        
        base_tree = DecisionTreeQuantileRegressor(
            criterion=criterion_mapped,
            max_depth=max_depth,
        ...
        try:
            # Try newer API with estimator parameter (sklearn 1.4+)
            ...
            # Fallback for older sklearn versions using base_estimator
            try:
                super(RandomForestQuantileRegressor, self).__init__(
                ...
                super(RandomForestQuantileRegressor, self).__init__(
                    ...
                    verbose=verbose,
                    warm_start=warm_start)
                # Store tree parameters to set in fit if needed
                self._tree_params = {
                    'criterion': criterion_mapped if 'criterion_mapped' in locals() else criterion,
                    'max_depth': max_depth,
                    'min_samples_split': min_samples_split,
                    'min_samples_leaf': min_samples_leaf,
                    'min_weight_fraction_leaf': min_weight_fraction_leaf,
                    'max_features': dt_max_features if 'dt_max_features' in locals() else max_features,
                    'max_leaf_nodes': max_leaf_nodes,
                }
...
class BaseTreeQuantileRegressor(BaseDecisionTree):
    ...
    def predict(self, X,u, check_input=False): # ,cos_sin
        """
        Predict cond. char. values for tree-based quantile regressor
        ...
        u : array-like of shape = [B, dim_y]
            Random vectors for characteristic function estimation
        ...
        Returns
        -------
        r_cos, r_sin : arrays of shape = [n_samples, B]
            Estimated cosine and sine components of conditional characteristic function
        """
        # apply method requires X to be of dtype np.float32
        ...
        for leaf in unique_leaves:
            ...
            r_cos[X_leaves == leaf,:] = np.mean(np.cos(y.dot(u.T)),axis = 0)
            r_sin[X_leaves == leaf,:] = np.mean(np.sin(y.dot(u.T)),axis = 0)
        return r_cos, r_sin
...
class DecisionTreeQuantileRegressor(DecisionTreeRegressor, BaseTreeQuantileRegressor):
    ...
    def __init__(self,
                 ...
                 random_state=None,
                 max_leaf_nodes=None):
        # Map deprecated criterion names for sklearn 1.3+ compatibility
        criterion_mapped = 'squared_error' if criterion == 'mse' else ('absolute_error' if criterion == 'mae' else criterion)
